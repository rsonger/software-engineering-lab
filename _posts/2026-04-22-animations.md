...
Uniform variables are useful when we want to send data directly from our Python apps to variables in the GPU program.
...
For example, we use `glUniform1i` for a single integer (or Boolean) value, but `glUniform3f` for a `vec3`. 
The last letter of the function name indicates the data type, with `i` meaning `int` and `f` meaning `float`. 
After completing these two steps, the uniform variable in the shader program will reference the given data every time the shader draws a frame.

## The Uniform Class

This time we will create the `Uniform` class to handle uniform variables similar to how we made the `Attribute` class for attribute variables last time. 
The responsibilities of the `Uniform` class include:  
- storing the data and data type of a specified uniform variable
- getting and storing a reference to the uniform variable
- loading data for the uniform variable as needed
...
The `Uniform` class stores the valid data types with a tuple and raises an exception if the given data type is not one of them. 
This makes it easier to debug your code if you have a typo in your application code or try to use a type that we do not support.  
...
That function will return `-1` if the variable cannot be found, which we handle by raising a `ValueError` with a descriptive message to the user. 
If we don't raise an error here, that `-1` value will create a more obscure error later when we try to use the variable reference.
...
The first thing we do is confirm that we have a reference to the uniform variable. 
When the `Uniform` object initializes, `self.variable_ref` is set to `None` but then it receives a value when the `locate_variable` method is called. 
So the error message is a reminder to call `locate_variable` before `upload_data`. 
Then we check the data type of this uniform variable and upload the data with the appropriate [`glUniform`](https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glUniform.xhtml){:target="_blank"} function. 
If the data stores more than one value (as with a `list` or `tuple`), we [unpack the data](https://docs.python.org/3/tutorial/controlflow.html#tut-unpacking-arguments) to pass its values as separate arguments to the GL function.
...
The application that tests the `Uniform` class will show two triangles with the same shape, but different positions and colors. 
For this purpose, we will use shader programs that are slightly modified from before. The new **vertex shader** code looks like this:
...
First, the `translation` variable has the `uniform` qualifier which identifies it as a uniform variable. 
We will use our `Uniform` class to upload data to `translation` when we write the test app.

Second, the value we assign to `gl_Position` combines the values of `position` and `translation`. 
The `position` variable will reference three vertices centered around the origin that together represent the triangle's shape. The `translation` variable stores values representing the change in position for the triangle. Combining `position` and `translation` will effectively move the center of the triangle before drawing it. We will use this approach so that the left triangle shifts $-0.5$ units on the $x$-axis while the right triangle shifts $0.5$ units.
...
It just sets the `fragColor` output using the components of the `baseColor` uniform variable which we will upload from our Python app when drawing each triangle. 
This way we can make the left triangle red and the right triangle blue by uploading their respective colors.

Our test app needs to create an `Uniform` instance for both the `translation` and `baseColor` uniform variables. 
We will set up each `Uniform` instance in the `startup` method, then upload the respective data for these variable before drawing each triangle in the `update` method.
...
class Test_4_1(WindowApp):
    ...
    def startup(self):
        ...
        shader_ref = initialize_program(vs_code, fs_code)
        ...
        position_attribute.associate_variable(shader_ref, "position")

        # create a Uniform object for the translation of each triangle
        self.left_translation = Uniform("vec3", [-0.5, 0.0, 0.0])
        self.left_translation.locate_variable(shader_ref, "translation")

        self.right_translation = Uniform("vec3", [0.5, 0.0, 0.0])
        self.right_translation.locate_variable(shader_ref, "translation")

        # create a Uniform object for the color of each triangle
        self.red_color = Uniform("vec3", [1.0, 0.0, 0.0])
        self.red_color.locate_variable(shader_ref, "baseColor")

        self.blue_color = Uniform("vec3", [0.0, 0.0, 1.0])
        self.blue_color.locate_variable(shader_ref, "baseColor")

        GL.glUseProgram(shader_ref)
...
This app saves a reference to each `Uniform` object associated with a uniform variable in the shader. 
Then we can reference the `Uniform` object in the `update` method and use its `upload_data` method to assign its value to the associated variable before drawing. 
Note that the data MUST be uploaded BEFORE calling `glDrawArrays` or the shader program will not use the correct data to draw.

## Animations

Until now, all of our test apps have used the `update` method to draw a still image in each frame, which it does 60 times a second. 
Remember that in the application runtime lifecycle, the `update` method runs in a continuous loop that updates data and renders the image at 60 FPS (see the `run` method in the `WindowApp` class). 
With static images, we get no real benefit from structuring our application in this way. 
The benefit comes when the image changes over time to create animations. 
Let's take a look at the essential techniques for rendering animated images.

### Clearing the Screen

One of the essential techniques in computer animation is to clear the screen in between frames. 
If we do not clear the screen, then the images drawn in the previous frame will remain on the screen and the new images will draw on top of them. 
Clearing the screen will effectively erase the previously drawn image and prepare the screen for drawing the new frame. 
OpenGL provides two functions for this:  
- The [`glClearColor`](https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glClearColor.xhtml){:target="_blank"} function sets a RGBA color to use when clearing the color buffer. 
The color provided will effectively become the background color for the new frame.
- The [`glClear`](https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glClear.xhtml){:target="_blank"} function resets a buffer specified by the parameter. 
OpenGL constants can be used for the parameters, such as `GL_COLOR_BUFFER_BIT`, `GL_DEPTH_BUFFER_BIT`, `GL_STENCIL_BUFFER_BIT`, or any combination of these using the bitwise OR `|` operator.
...
class Test_4_2(WindowApp):
    ...
    def startup(self):
        ...
        self.translation.locate_variable(shader_ref, "translation")

        self.base_color = Uniform("vec3", [1.0, 0.0, 0.0])
        self.base_color.locate_variable(shader_ref, "baseColor")
...
Since this app only draws a single triangle, it only needs one `Uniform` object for the `translation` variable and one for the `baseColor` variable. 
Here we use a list for the data of each `Uniform` object because Python tuples are *immutable* (i.e., they cannot change). 
If we used a tuple instead, we would not be able to change the data values (and would not be able to animate the triangle).

In the `update` method, we directly set the new data for the $x$-coordinate translation in the `Uniform` object before uploading its data. 
If the triangle moves more than `1.2` units to the right, we reposition it to the left side of the screen. 
Remember, the leftmost point in the triangle position data is at `-0.2` on the $x$-axis, so a translation value of `1.2` will put it at exactly `1.0`, the edge of the screen.

### Keeping Time

Another essential technique in computer animation is to calculate movements based on the amount of time that has passed since the previous frame. 
For this purpose, we will introduce two new instance variables in `WindowApp` for keeping track of time:
...
These variables will go in the `WindowApp` class so they can be inherited into each of our apps. 
...
Here, the double underscores in the variable names will give complete control of the variables to the `WindowApp` class. 
That means our apps could create their own `__time` and `__delta_time` variables without overwriting the timekeeper features in `WindowApp`.
...
This makes the `time` and `delta_time` variables read-only so apps cannot change their values.
...
Now let's test the timekeeper variables with an app that calculates the triangle's position based on time. 
We could make the triangle move back and forth across the screen with a simple linear equation, or we can use sine and cosine functions to make the triangle move more smoothly between $1$ and $-1$ values. 
If we assign cosine and sine functions to the $x$ and $y$ coordinates respectively, we can calculate a circular path from the time variable $t$:
...
This gives coordinates for a circular path centered at the origin $(0,0)$ with a radius of $1.0$. 
We can change the radius to some value $r$ and make the center some coordinate $(a,b)$ by altering the formulas a little bit:
...
If the triangle's path has a radius of $1.0$, then its corners will run off screen at the edges. 
Let's reduce the radius to $0.75$ to keep the entire triangle on screen at all times.

:heavy_check_mark: ***Try it!***  
<input type="checkbox" class="checkbox inline"> Copy your `test_4_2.py` file and change its name to `test_4_3.py`.  
<input type="checkbox" class="checkbox inline"> At the top of the file, change the filename comment and add an `import` statement to get functions for sine and cosine:  
...
<input type="checkbox" class="checkbox inline"> Find the lines with the class definition, document string, and startup print statement then change them to the following:
...
<input type="checkbox" class="checkbox inline"> On the last line of the file, change the code that runs the app to `Test_4_3().run()`.  
<input type="checkbox" class="checkbox inline"> Save the file and run the app with the `python test_4_3.py` command in your terminal.  
...
This app uses `self.time` as the number of radians for the sine and cosine calculations, so the triangle will complete one full revolution every $2 \pi$ seconds.

The final test app for animations will use our timekeeper variables to animate changing color. 
We will copy `test_4_3.py` and use the sine and cosine functions again, but this time we will change the value of the `baseColor` shader variable instead of `translation`.

Color values must be in the range of $[0.0,1.0]$ but sine results are in the range $[-1.0,1.0]$, so we need to change our equation again. 
We can do this by shifting the range of results to include only positive values with $sin(t) + 1$. 
Then we multiply this by $0.5$ to shrink the range of values to half and produce the final formula $f(t) = 0.5(sin(t)+1)$.
...
The green and blue values of the triangle color are stored in `self.base_color.data[1]` and `self.base_color.data[2]`, respectively. 
If we wanted to shift through a wider range of colors, we would need to update those values also. 
But the equations for the green and blue values would need to be different so they peak at different times. 
In order to visualize this, it may be helpful to see a graph. 
[Here](https://www.desmos.com/calculator/uymwhpwc7x){:target="_blank"} is a graph at Desmos.com which shows one simple approach for oscillating RGB values. 
Try changing the expressions to see what different effects you can create!

# Adding Interactivity

Up to now, our applications have only allowed one kind of input&mdash;closing the window. 
If our framework also allows for keyboard inputs, we will be able to support a much wider range of interactive features.  

## Keyboard Input with Pygame

In our current framework design, we handle input events with the `update` method of the `Input` class. 
We use Pygame to detect input events and process quit type events. 
Now let's add the ability to handle keyboard events in the `Input` class. 
There are two types of keyboard events that Pygame recognizes:

- **Keydown events** happen when a key is first pressed down.
- **Keyup events** happen when a pressed key is released.

These Pygame events are *discrete*, which means they happen only in a single moment. 
However, some user actions are *continuous* which means they happen over a period of time (such as holding down an arrow key or click-and-drag actions). 
In order to handle *continuous* keyboard inputs in addition to *discrete* ones, we will use Python sets to keep track of each key's state. 
Sets are useful here because we know each key is unique and the order we store them does not matter.  
...
The **keydown** and **keyup** events represent *discrete* states, so we will clear their sets at the start of each update cycle. 
The `pressed_keys` set holds keys in a *continuous* state, so their names will remain in the set until their **keyup** event is received.
...
Again, we want only the `Input` class to manage these sets, so we name them with double underscores (`__`). 
...
Now, it is common for applications to have a keyboard shortcut for closing the application. 
Currently, our `quit` property is read-only so we cannot use it to close an application with the `Input` class. 
Let's add a **setter** property for `quit` to give some control back to the applications.
...
Here we convert the given `value` to a `Boolean` type and assign it to `_quit`. 
This way we can make sure that the values assigned to the `_quit` variable will always be `True` or `False`.

<input type="checkbox" class="checkbox inline"> Find the `update` method inside the `Input` class and add the following code just before the `for` loop in the `update` method.  
...
Now that we have an  `Input` class with working keyboard features, let's use them to control the movements of a triangle on the screen. 
We will begin by changing our first test app with animation `test_4_2.py` so that its `update` method allows control of the triangle's movement with keyboard input.
...
<input type="checkbox" class="checkbox inline"> Find the lines with the class definition, docstring, and startup print statement then change them to the following:
...
:warning: **NOTE:** Different operating systems might use different names for their key events. 
For example, `"left"` might be `"left arrow"` instead. 
This `test_4_5.py` app will be a useful tool for you in the future when you want to know which names are associated with which keys.

Now that we have all the basic components for drawing and interacting with animated shapes, we can turn our focus to creating 3D graphics. 
The next lesson will begin with a review of vector and matrix math concepts, which are essential to 3D CG. Look forward to it!